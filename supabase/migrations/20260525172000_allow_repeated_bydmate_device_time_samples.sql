-- Allow BYDMateOWN to store repeated cloud samples with the same device_time.
-- The car can report an unchanged device timestamp across background polls;
-- deduping only by device_time made those polls disappear until a manual test
-- refreshed the vehicle payload.

drop index if exists public.bydmate_telemetry_samples_user_vehicle_device_unique;
