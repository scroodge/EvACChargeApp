-- Charging samples describe a parked EV state. Keep them in live/history
-- telemetry, but do not let them create or extend driving trips.
...
begin
  ...
  if v_is_charging then
    ...
    return jsonb_build_object(
      'charging', true,
      'trip_id', null,
      'closed_trip_id', case when found then v_trip.id else null end,
      'sample_count', 0,
      'track_point_count', 0
    );
  end if;
